...
AddEventHandler('keep-hunting:client:slaughterAnimal', function(entity)
    ...
    if (model and animal) then
        local hasKnife = CoreName.Functions.HasItem("weapon_knife")

        if hasKnife then
            ClearPedTasks(PlayerPedId())
            ToggleSlaughterAnimation(true, entity)
            CoreName.Functions.Progressbar("harv_anim", "Slaughtering Animal", Config.SlaughteringSpeed, false, false, {
                ...
                TriggerServerEvent('keep-hunting:server:AddItem', animal, entity, multiplier)
                Citizen.Wait(100)
            end)
        else
            CoreName.Functions.Notify("You don't have a knife.")
        end
